...
std::optional<std::vector<std::shared_ptr<MeshAsset>>> loadGltfMeshes(VulkanEngine* engine, const std::filesystem::path& filePath) {
    ...
    for (fastgltf::Mesh& mesh : gltf.meshes) {
        ...
        for (auto&& p : mesh.primitives) {
            GeoSurface newSurface{}; // triangle in cube or suzanne
        ...
        newmesh.meshBuffers = engine->uploadMesh(indices, vertices); // uploaded suzanne/cube to gpu

        meshes.emplace_back(std::make_shared<MeshAsset>(std::move(newmesh))); // add suzanne/cube to cpu list
    }

    return meshes; // return all meshes including suzanne + cube
}
